...
class AdminHomePageController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // Handle feature background
        if($request->feature_background != null) {
            $request->validate([
                'feature_background' => 'image|mimes:jpg,jpeg,png',
            ]);
            
            // Check and unlink old file if it exists
            if ($home_page_item->feature_background != null) {
                $filePath = public_path('uploads/'.$home_page_item->feature_background);
        ...
        if($request->testimonial_background != null) {
            ...
            if ($home_page_item->testimonial_background != null) {
                $filePath = public_path('uploads/'.$home_page_item->testimonial_background);
                if (file_exists($filePath)) {
                    unlink($filePath);
                }
            }
